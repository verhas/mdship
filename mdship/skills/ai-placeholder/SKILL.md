...
3. Read the existing content in the section as context — it may be partially correct or useful as a starting point.
4. Generate content based on the `prompt`, the document's surrounding context, style, and heading level.
5. Replace only what needs changing. Preserve wording, structure, or examples from the existing content where they are still accurate and fit the prompt — avoid rewriting for its own sake.
6. Write the result between the opening placeholder and the closing marker, replacing the old content, leaving both markers unchanged.
