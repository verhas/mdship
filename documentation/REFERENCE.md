...
The complete reference for mdship's commands and placeholders. For what mdship is for and a quickstart, see the [README](../README.md); each command also has its own page under [commands/](commands/).

<!--TOC min-level: 2
max-level: 3
_terminate_: "TIC"
_content_generated_: 1181:md5:9f221aa75a505e00b20bf760c25a61b0
# ⚠️ MANAGED CONTENT: Edits will be lost.
# danger zone: Delete _content_generated_ to override.
-->
- [1. Markdown Commands](#1-markdown-commands)
  - [1.1. Command Line](#11-command-line)
  - [1.2. Shift Validation](#12-shift-validation)
  - [1.3. Shift Line Range](#13-shift-line-range)
  - [1.4. Semantic Line Breaks](#14-semantic-line-breaks)
  - [1.5. Number Headings](#15-number-headings)
  - [1.6. Checksums](#16-checksums)
  - [1.7. Skipping Backups](#17-skipping-backups)
  - [1.8. Tracking Changes](#18-tracking-changes)
  - [1.9. Validating Links](#19-validating-links)
- [2. Placeholders](#2-placeholders)
  - [2.1. Placeholder Processing Order](#21-placeholder-processing-order)
  - [2.2. Variables](#22-variables)
  - [2.3. Template Placeholders](#23-template-placeholders)
  - [2.4. Table of Contents](#24-table-of-contents)
  - [2.5. Including Files](#25-including-files)
  - [2.6. Rendering Mermaid Diagrams](#26-rendering-mermaid-diagrams)
  - [2.7. Placeholder Validation](#27-placeholder-validation)
  - [2.8. Managed Content Integrity](#28-managed-content-integrity)
  - [2.9. AI Placeholders](#29-ai-placeholders)
- [3. Integrations](#3-integrations)
  - [3.1. MCP Server](#31-mcp-server)
  - [3.2. GitHub Action](#32-github-action)
- [4. Design](#4-design)
<!--/TIC-->
...
Most commands modify the file in place. Before writing, they save the previous content to a `.md.bak` file, unless git already holds it (see [Skipping Backups](#17-skipping-backups)):
...
### 2.1. Placeholder Processing Order
...
4. **<!​--JINJA2-->** and the deprecated **<!​--TEMPLATE-->** placeholders
...
- JINJA2 (and the deprecated TEMPLATE) to use any variables including those in included content
...
#### 2.2.1. IMPORT: Load from External Files
...
#### 2.2.3. SIP: Extract Predefined Variables from Files
...
#### 2.2.4. SUP: Extract from Document Lines
...
> **Deprecated:** `TEMPLATE` is deprecated and will be removed in a future release; existing `TEMPLATE` placeholders keep working until then. Use [`JINJA2`](#231-jinja2-template-placeholders) instead: rename the markers to `<!​--JINJA2-->` / `<!​--/JINJA2-->` and replace each `$variable` or `${variable}` with `{{ variable }}` (`$config.port` becomes `{{ config.port }}`, `$authors[0]` becomes `{{ authors[0] }}`). Two differences to check when migrating: an undefined variable is left as literal `$name` text by `TEMPLATE` but rendered as an empty string by `JINJA2`, and `TEMPLATE` adds a blank line after the generated content where `JINJA2` does not.
...
#### 2.3.1. Jinja2 Template Placeholders

Use `JINJA2` when the generated content needs real template logic such as loops, conditionals, filters, or nested object access. The rendered result is inserted between the opening and closing markers and protected as managed content. `JINJA2` replaces the deprecated `TEMPLATE` placeholder.
...
Use `JINJA2` for all new templates, including simple variable substitution: `{{ name }}`.
...
### 2.6. Rendering Mermaid Diagrams
...
-->
content
<​!--/JINJA2X-->  ← Error: Should be <!--/JINJA2-->
```

Error message:
```
Line 6: Closing <​!--/JINJA2X--> does not match opening <​!--JINJA2--> at line 1. Is there a typo in the closing tag? Expected <!--/JINJA2-->
```

```markdown
# ❌ Unclosed JINJA2 placeholder
<​!--JINJA2
content: |
  Test
-->
...
Error message:
```
Line 1: Unclosed <!--JINJA2--> placeholder. Expected closing tag <!--/JINJA2-->
```
...
from: a
-->
<​!--/JINJA2-->  ← Should be <!--/INCLUDE-->
<​!--/INCLUDE-->
```

Error message:
```
Line 7: Closing <!--/JINJA2--> does not match opening <!--INCLUDE--> at line 4. Is there a typo in the closing tag? Expected <!--/INCLUDE-->
```

### 2.8. Managed Content Integrity
...
**Applies to:** `<!--TOC-->`, `<!--INCLUDE-->`, `<!--MERMAID-->`, `<!--JINJA2-->`, the deprecated `<!--TEMPLATE-->`, and PYTHON `run:` — all built-in placeholders that generate managed content. Each placeholder processor must explicitly wire in the hash check; it is not applied automatically to new placeholders.
...
`ai-check` compares against checksums recorded by `ai-fix` or `ai_update`. A placeholder that has never been generated has nothing recorded, so `ai-check` passes it; use `mdship ai-list` to find entries with status `never_generated`. Nothing is enforced unless a check runs, so run `ai-check` in CI (see [GitHub Action](#32-github-action)).

**Validate AI placeholder names:**

The `validate` command also checks AI placeholder name uniqueness:

```bash
mdship validate file.md
```

For full details on deps, brief, the MCP tools (`ai_context`, `ai_update`), and the Claude workflow see [documentation/AI.md](AI.md).
...
For a step-by-step cookbook with copy-paste workflow recipes see [documentation/GITHUB_ACTIONS.md](GITHUB_ACTIONS.md).
